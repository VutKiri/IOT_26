...
class BG77:
    ...
    def setOperator(self, mode = COPS_AUTO, op_plmn = Operator.CZ_VODAFONE) -> bool:
        ...
        if mode == COPS_AUTO:
            ...
            raise OSError("BG77 Invalid COPS mode.")
...
class BG77Socket:
    ...
    def __init__(self, bg77: BG77, socket_num, socket_class, socket_protocol, socket_mode, socket_push_mode) -> None:
        ...
        self.tx_format = FORMAT_PLAIN_TEXT
        self.rx_format = FORMAT_PLAIN_TEXT
    ...
    def setDataFormats(self, tx_format = FORMAT_PLAIN_TEXT, rx_format = FORMAT_PLAIN_TEXT):
        ret = self.modem.sendCommand(f"AT+QICFG=\"dataformat\",{tx_format},{rx_format}\r\n",exit_condition="OK\r\n",timeout = 3)
        if not "OK" in ret:
            return False
        self.tx_format = tx_format
    ...
    def recvBytes(self, num_bytes=100):
        #No timeout set
        if not (self.rx_format == FORMAT_HEX):
            print("Error receiveing Bytes. Format not set to hex mode. Use setDataFormats().")
            return False
        if self.__dataInBuffer():
            data_len, message = self.__readFromBuffer(num_bytes*2)
            return data_len, message
        #Inifinite buffer timeout - wait forever
        if self.timeout == -1:
            while True:
                ...
                if "\"recv\"" in ret:
                    ...
                    if int(ret_split[0]) == self.socket_num:
                        if self.socket_push_mode == SOCK_PUSH_TERMINAL:
                            data_len = int(ret_split[1])
                            message_response = self.modem.__read(exit_condition="\r\n")
                            message = message_response[:(data_len*2)]
                            return data_len, message
                        elif self.socket_mode == SOCK_PUSH_BUFFER:
                            data_len, message = self.__readFromBuffer(num_bytes*2)
                            return data_len, message
    ...
    def __readFromBuffer(self,data_size):
        ...
        ret = ret.strip("+QIRD: ")
        data_len = int(ret)
        if self.rx_format == FORMAT_HEX:
            data_len = data_len*2
        #message_raw = self.modem.__read(timeout=2)
        message_raw = self.modem.serial.read(data_len).decode()
        if self.modem.verbose:
        ...
        self.modem.__read()
        if self.rx_format == FORMAT_HEX:
            return int(data_len/2), bytes.fromhex(message)
        return data_len, message
